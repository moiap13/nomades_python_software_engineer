...
cred = credentials.Certificate(os.path.join(PROJECT_DIR, 'config', "firestore-creds.json"))
...
def register():
  ...
  if request.method == "POST" and register_form.validate():
    ...
    # TODO: Use firestore collection instead of csv file
    ...
    for user in users:
      ...
      if (
        user_data.get("uid", "").lower() == uid.lower()
        or user_data.get("email", "") == email.lower()
      ):
        error = "User ID or Email already exists"
        return render_template('login/register.html', form=register_form, error=error)
    ...
    #   # # TODO: Add docuemnt to firestore using add() function
    ...
    user: User = User(uid, pwd_h, salt, pp_filename, firstname, lastname, email, age)
    db.collection(u"users").add(user.to_dict(True))
...
def login():
  ...
  if request.method == "POST":
    ...
    # TODO: get all the users from firestore and loop to find a matching login
    # with open(CSV_FILE, "r") as users_csv_file:
    #   users = csv.DictReader(users_csv_file)
    #   for user in users:
    ...
    users = db.collection(u"users").stream()
    for user in users:
      user_obj: User = User.from_dict(user.to_dict())
      if user_obj.uid == uid_email or user_obj.email == uid_email:
        salt: str = user_obj.salt
        pwd_h: str = hashlib.sha256((pwd+salt).encode()).hexdigest()
        if user_obj.pwd == pwd_h:
          session["loggedin"] = True
