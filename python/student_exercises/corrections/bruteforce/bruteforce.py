...
def brute_force_password(charset: str, length: int, target_hash: str):
    MAX_ITER: int = len(charset) ** length
    start_time = time.time()

    for comb in itertools.product(charset, length):
        if target_hash == (p := hashlib.sha256(comb.encode()).hexdigest()):
    ...
    #         p += charset[iter // len(charset)**exp % len(charset)]
    #     password: str = hashlib.sha256(p.encode()).hexdigest()
    #     # print(p, password, sep="\t\t")
    #     if password == target_hash:
    #         # you found the password hourra
    #         return p, time.time() - start_time

    # for c1 in charset: # 26
    #     for c2 in charset: # 26**2
    #         for c3 in charset: # 26**3
    #             for c4 in charset: # 26**4
    #                 for c5 in charset: # 26**5
    #                     p: str = c1+c2+c3+c4+c5
    #                     password: str = hashlib.sha256(p.encode()).hexdigest()
    #                     print(p, password, sep="\t\t")
    #                     if password == target_hash:
    #                         # you found the password hourra
...
# Example usage:
charset = string.ascii_lowercase + string.ascii_uppercase + string.digits + string.punctuation  # Define your character set
